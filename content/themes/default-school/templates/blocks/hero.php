<section class="hero">
    ...
    <div class="container hero-inner">
        ...
        <p class="hero-badge"><?= htmlspecialchars((string) $props['badge']) ?></p>
        <?php endif; ?>
        <h1><?= htmlspecialchars((string) (($props['title'] ?? '') !== '' ? $props['title'] : $site_name)) ?></h1>
        <?php if (!empty($props['subtitle'])): ?>
        <p class="hero-subtitle"><?= htmlspecialchars((string) $props['subtitle']) ?></p>
        <?php endif; ?>
        ...
        <div class="hero-links">
            <?php foreach ($props['buttons'] as $btn): ?>
                ...
                <?php $style = ($btn['style'] ?? 'primary') === 'outline' ? 'btn-outline' : 'btn-primary'; ?>
                <a href="<?= htmlspecialchars(route(ltrim((string) ($btn['url'] ?? ''), '/'))) ?>" class="btn <?= $style ?>">
